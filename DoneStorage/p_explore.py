...
def p_explore():
    ...
    doc_dic, functions_dic, fullname_dic, parameters_dic, url_dic = pars_functions.dis_dictionaries()
    ...
    with make_expanders("Выбрать распределение"):
        # функция имен
        display = pars_functions.dis_name()
    ...
    class Figure:
        ...
        global_rc_params = {'legend.fontsize': 12,
        ...
        def pdf_cdf_lines(self, ax):
            ...
            # Постройте замороженный PDF-файл, если флажок активен
            if select_pdf:
                ax.plot(self.x, self.rv.pdf(self.x), linestyle='-',
                        color=self.colors['pdf_line_color'],
                        lw=1, label='PDF')
                # Установите сияние, если флажок активен
                if select_pdf_shine:
    ...
    def how_many_params():
        ...
        if len(sliders_params) == 2:
            ...
            for i, param in enumerate(sliders_params[0:-2]):
                param_name = pars_functions.stats_name().get(f'{select_distribution}').shapes.split(', ')
                name = f'{param_name[i]}'
